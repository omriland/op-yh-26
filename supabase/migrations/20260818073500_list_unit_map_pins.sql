-- Unit map pins: every signed-in active user can read map-visible
-- addresses (name, callsign, kind, coords, volunteer status, availability)
-- without opening profiles or user_addresses to responders.
...
revoke all on function public.list_unit_map_pins() from public;
grant execute on function public.list_unit_map_pins() to authenticated;
