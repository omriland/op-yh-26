...
/** Lucide Phone — never mirror (handset). */
...
function appendPhoneIcon(host: HTMLElement) {
  const ns = 'http://www.w3.org/2000/svg'
  const svg = document.createElementNS(ns, 'svg')
  svg.setAttribute('class', 'user-map-pin__phone')
  svg.setAttribute('viewBox', '0 0 24 24')
  svg.setAttribute('aria-hidden', 'true')
  svg.setAttribute('fill', 'currentColor')
  const handset = document.createElementNS(ns, 'path')
  handset.setAttribute('d', PHONE_PATH)
  svg.append(handset)
  host.append(svg)
}
  ...
  class LabeledPin extends maps.OverlayView {
    ...
    onAdd() {
      ...
      if (isKm) {
        ...
        if (extraClass === 'user-map-pin--phone') appendPhoneIcon(text)
        const name = document.createElement('span')
        name.className = 'user-map-pin__name'
    ...
    setCopy(nextLabel: string, tooltipText?: string) {
      if (!this.el) return
      const name = this.el.querySelector('.user-map-pin__name')
      const text = this.el.querySelector('.user-map-pin__label')
      if (name) name.textContent = nextLabel
      else if (text) text.textContent = nextLabel
