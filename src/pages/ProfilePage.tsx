...
const VEHICLES_VIEW_ONLY_CAPTION =
  'רכבים מנוהלים על ידי מנהל המערכת. לבחירת רכב ראשי לחצו על הכוכב.'
        ...
        <section className="card">
          ...
          <div className="stack-4" style={{ marginBlockStart: 'var(--space-4)' }}>
            {vehicles === null ? (
              ...
              <p className="t-body text-muted">עדיין לא רשומים רכבים. פנו למנהל המערכת להוספת רכב.</p>
            ) : (
              vehicles.map((vehicle) => (
                <Ledger key={vehicle.key}>
                  <LedgerRow
                    label={vehicle.archived ? `${vehicle.model} (בארכיון)` : vehicle.model}
                    value={
