...
- **Android OTA updates (2026-09-03):** Sideloaded app installs its own updates — no Play Store, no browser download per release. `/android/version.json` gains `apkSha256` + `apkSizeBytes`; Android downloads to app cache, verifies the digest, then commits a `PackageInstaller` session (system Install sheet still appears once — silent install needs MDM). Modes: `current < min` = force block, `min ≤ current < latest` = dismissible soft prompt. Regenerate the digest with `npm run android:checksum` (`--check` fails when stale); release scripts in `yahpaz-android` write it too and honor `MIN_VERSION_CODE`. Spec: `2026-09-03-yahpaz-android-ota-updates-design.md`. Android code delivered as `docs/superpowers/patches/2026-09-03-yahpaz-android-ota.patch` (agent cannot push to `yahpaz-android`). **Bootstrap:** old clients need one last browser-based force update onto the OTA-capable build (versionCode 25 / 0.3.14) before in-app OTA works.
