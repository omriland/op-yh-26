...
- **Freeze deploy order (2026-09-11):** **Migration must land before the web deploy.** Probed prod REST 2026-09-11: `event_responders.frozen_over_60km` → 400 `42703` (missing); `events.approved_over_60km_responder_ids` / `approved_suspicious_duplicate` / `frozen_over_60km` → 200 (present, so the migration's prerequisites are met). Merging PR #45 to `infra/bootstrap` first would 400 every events/cockpit/shifts/report select. Sequence: run `20260910140000_responder_level_event_freeze.sql` (idempotent) in the Supabase SQL editor → merge #45 → Netlify Git CD. Branch `cursor/per-responder-event-freeze-921a` @ `34a1cc2` is build-green (`npm run build`) and 1435 tests pass (`TZ=Asia/Jerusalem`). Cloud-agent secrets hold only `VITE_*`, so an agent still cannot apply migrations or run `netlify deploy`.
