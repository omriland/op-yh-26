...
- Invite link TTL: **24 hours** (`profiles.invite_token_expires_at`; Edge `admin-users` `INVITE_TTL_MS`). Email body notes `הקישור בתוקף ל־24 שעות.` Expired → `קישור ההזמנה פג תוקף. בקשו הזמנה חדשה.` Admin resend mints a fresh token.
