...
- List is grouped by logging state, not by date: `אירועים הממתינים לתיעוד` (always shown; empty copy `מברוק! אין לך עוד אירועים לתעד כרגע`) then `אירועים שתועדו` (last 30 days; secondary `הצג 30 יום נוספים` loads another 30-day window).
