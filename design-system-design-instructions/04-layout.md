...
- **Desktop Command chrome:** no top bar, on any screen. Two floating cards on Field `--surface-sunken`, gutter `--space-3`: sidebar card at the **inline-start** (right side in RTL), default 240 px wide (user-resizable 190–265 px via invisible edge handle; preference in `localStorage` `yahpaz.sidebarWidth`), Command `--surface-page`, hairline, `--radius-lg`; **content card beside it is Field** (`data-theme="field"` on `.shell__main`, Field `--surface-page`, same hairline + radius). Wordmark sits at the top of the sidebar; availability + user menu pin to the sidebar footer. Forms, detail, fill, and cockpit keep the same sidebar.
